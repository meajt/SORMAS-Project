...
public class AdditionalTestForm extends AbstractEditForm<AdditionalTestDto> {
    ...
    private void addMediatorLogic() {
        TriFunction<Object, String, Object, Boolean> healthScreeningMediator = (Object source, String propertyId, Object value) -> {
            if (source != lipidProfileSampleCreateForm)
                lipidProfileSampleCreateForm.applyEvent(propertyId, value);
            if (source != healthScreeningTestForm)
                healthScreeningTestForm.applyEvent(propertyId, value);
            if (source != urineRoutineExaminationForm)
                urineRoutineExaminationForm.applyEvent(propertyId, value);
            if (source != healthScreeningTestForm.getUrineRoutineExaminationForm())
                healthScreeningTestForm.getUrineRoutineExaminationForm().applyEvent(propertyId, value);
            return true;
        };
        healthScreeningTestForm.setEventSender(healthScreeningMediator);
        lipidProfileSampleCreateForm.setMediator(healthScreeningMediator);
        urineRoutineExaminationForm.setEventSender(healthScreeningMediator);
    }
    ...
    private CheckBox addNcdFieldForm(String checkBoxId, String fromId, Component form) {
        ...
        form.setWidth(form.getWidth() * 8 / 12, Unit.PIXELS);
        return checkBox;
    }
    ...
    public void updateNcdSampleFormValue() {
        ...
        if (value.getLipidProfileSampleDto() != null) {
            ...
            healthScreeningTestForm.setFasting(value.getLipidProfileSampleDto().getFasting());
            healthScreeningTestForm.setPostParaial(value.getLipidProfileSampleDto().getPostParandial());
        } else {
    ...
    public void updateNcdSampleDtoValue() {
        ...
        //healthScreeningTestForm.commit();
    }
