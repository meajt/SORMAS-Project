...
public class NewsFacadeEjb implements NewsFacade {
   ...
   private final String baseUrl = "http://202.166.212.180/api";
   private final String authToken = "1|s0xqFm96hDsdZSukBHUp2AsLkWS4i4EELVFEv8uR";
    @Override
    public long count(NewsCriteria criteria) {
        RestClient restClient = new GsonRestClient(gson)
                .setAuthHeader(String.format("Bearer %s", authToken));
        ...
        NewsBodyResponse response = restClient.get(baseUrl+NEWS, filterQueryMap, NewsBodyResponse.class);
    ...
    public List<NewsDto> getNewList(NewsCriteria criteria, Integer first, Integer max, List<SortProperty> sortProperties) {
        ...
        RestClient restClient = new RestClient()
                .setAuthHeader(String.format("Bearer %s", authToken));
        String responseStr =  restClient.get(baseUrl+GET_NEWS, getFilterQueryMap(criteria, first, max),String.class);
